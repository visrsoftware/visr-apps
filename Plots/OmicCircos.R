...
visr.biocLite("GenomicRanges")
source("Plots/OmicCircosLib.R")

input_hlcolshade <- "#CCFF001A"
input_hlcolborder <- "#FF000080"

if (input_name == "") {
    error_message<-"No chromosome column is specified.\n[ Parameters -> Segments -> Name ]"
}

if (input_start == "") {
    error_message<-"No chromosome start position is specified\n[ Parameters -> Segments -> start ]"
}

if (input_end == "") {
    error_message<-"No chromosome end position is specified.\n[ Parameters -> Segments -> end ]"
}

if (input_position == "") {
    input_position <- input_start
    error_message<-"Warning: chromosome start position is used as chromosome position."
}
...
if (par()$fin[1] <= par()$fin[2]) {
    winx <- 800
    winy <- 800*par()$fin[2]/par()$fin[1]
}

if(par()$fin[1] > par()$fin[2]) {
    winx <- 800*par()$fin[1]/par()$fin[2]
    winy <- 800
}
