      ...
      if (cir.m == 0){
        ...
        for (chr.i in c(1:chr.num)){
          ...
          if (print.chr.lab){
            w.m <- (w1+w2)/2;
            r.j <- r/20;
            chr.t <- paste("   ", gsub("chr", "", chr.po[chr.i,1]), "   ");
            draw.text.rt(xc, yc, r+r.j, w.m, chr.t, cex=cex_label);
          } 
  ...
  do.scale.cir <- function (xc=xc, yc=yc, the.r=the.r, total.num=total.num, 
    ...
    for (j in 1:scale.n){
      ...
      draw.text.rt(xc, yc,  the.r+6, w, paste("  ",po.s,"  "), col=col, cex=cex_tick);
